# RAG-02 — Architecture & Design (corbidev-auth)
...
- Alternative: ORY Hydra si besoin modulaire; démarrage sur Keycloak recommandé.
...
- Apps Symfony: MariaDB (Doctrine ORM); migrations via `doctrine/migrations`.
- Keycloak: Postgres recommandé; MariaDB possible mais non idéal.

## Reverse Proxy & Réseau

- Nginx/Traefik/Caddy avec HTTPS obligatoire, HSTS, HTTP/2.
- Entêtes proxy: `X-Forwarded-*`; CSP adaptée à AssetMapper.

## Sécurité

- Password hashing: Argon2id; politique mots de passe conforme normes actuelles et vérification Pwned Passwords.
- Rate limiting: `symfony/rate-limiter` par IP et compte.
- 2FA: préparer TOTP (non obligatoire initialement).

## OIDC — Clients & Scopes

- Clients: Public (PKCE) pour web; Confidentiel pour backends.
- Scopes: `openid email profile`; claims standard + `roles` si géré par IdP.
- SLO: optionnel via Keycloak; commencer avec logout applicatif.
